...
//
// Being an address rather than a scan target, it is worth keeping: dhtAddress derives one
// from the wallet seed, the same on every run, so a sender can save it the way they save a
// phone number instead of being read a fresh one per payment. ephemeralAddress is the
// alternative for a run that should leave nothing reusable behind — see both below for
// which to reach for.
...
// This wallet's reusable address: same seed, same key, so `cashme get --dht` prints the
// same thing every run and a sender can keep it.
//
...
// is closer to giving out a phone number than a one-time code. ephemeralAddress is the way
// to be paid without leaving that behind.
//
...
// side as conn.remotePublicKey. Giving it the same dhtAddress we listen on makes a wallet
// one identity on both sides, which is what lets a receiver recognise a sender it has been
// paid by before — and, equally, what lets everyone we pay recognise us later.
// ephemeralAddress is the way to pay without that.
//
...
//
// `keyPair` is the address to listen on — dhtAddress or ephemeralAddress. `cancelled`
