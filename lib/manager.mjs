...
export async function openWallet(dir, { wait = false } = {}) {
  ...
  // stay reserved until someone reclaims them (see pendingSends).
  let manager
  try {
...
export async function mintTokens(wallet, mintUrl, amount, { unit = DEFAULT_UNIT, onQuote } = {}) {
  ...
  //
  // All of it goes through process.stderr rather than console.error: the two are separate
  // write paths in Bare and do not interleave in order, which lands the dots inside the
  // line that follows them.
  let waiting = false
  const note = (msg) => process.stderr.write(`${msg}\n`)
  const endWaiting = () => {
    if (waiting) process.stderr.write('\n')
    waiting = false
  }
...
// away from, or a bluetooth run that never got its ACK. Their proofs stay reserved, and
// out of the spendable balance, until one of the two below settles them.
