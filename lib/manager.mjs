...
export async function openWallet(dir, { wait = false } = {}) {
  ...
  try {
    ...
    return {
      ...
      async close() {
        // dispose() owns coco's background work, while repos.close() owns the process-wide
        // wallet lock. They are independent cleanup duties: a watcher failing to dispose must
        // never leave the wallet locked for the next command.
        try {
    ...
    // Seed creation, coco startup and recovery all run under the wallet lock. Any one may
    // fail after a manager has started its watchers, so tear down both layers without hiding
    // the error that stopped the open.
...
async function abandonWallet(manager, repos) {
  ...
  try {
    ...
    // close() releases the lock in a finally block. The opening error is the useful one,
    // while this is only diagnostic context for a failed cleanup write.
...
// Every mint this wallet has a record of, trusted or not. A mint stays on the list once
// untrusted rather than disappearing: its proofs are still here, and the way to spend them
// again is to find it and trust it back.
...
// Trust a mint by name rather than by having been paid by it.
//
// addMint is the half that reaches the network — it fetches the mint's info and keysets,
// which is also how a url that is not a mint at all is found out here rather than at the
// first send. trustMint is the half that records the decision, and it is called
// unconditionally because addMint's own flag does not move a record that already exists.
...
// Withdraw that trust. The proofs stay — coco keeps them and their keysets — but nothing
// can be spent from the mint until it is trusted again, which is what makes this the
// reversible way to quarantine a mint rather than a way to get rid of one.
export async function untrustMint(wallet, mintUrl) {
  ...
  try {
    ...
    // coco records the decision and then refreshes the mint's info, and that refresh
    // reaches the network whenever the cached copy is older than its five minute TTL. So an
    // unreachable mint throws *after* the untrust has already been written — and a mint you
    // cannot reach is exactly the one you most want to be able to quarantine. What matters
    // is whether the flag moved, not whether we managed to ask the mint about itself on the
    // way past.
...
export async function mintDetails(wallet) {
  ...
  // whose whole holding is reserved would otherwise not be listed at all — plus every mint
  // on the trust register, which is how one that has been untrusted, or trusted by name and
  // never used, is still something the wallet can show and act on.
  ...
  return [...mints].map((mintUrl) => {
    ...
    return {
      mintUrl,
      // A mint we hold proofs at but have no record of should read as untrusted rather than
      // as trusted-by-default: this figure decides whether a send from it can be made.
