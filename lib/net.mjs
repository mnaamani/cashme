...
// Why the OTA updater is not being started this run, or null. The updater is a detached
// process that fetches over the hyperdht, started with nothing but a storage directory (see
// App.spawnUpdater), so it knows none of what this run was told.
//
// --dht-interface stops it, and is the one flag whose promise the updater really would
// break: the updater reaches the hyperdht, which is exactly what the flag pins, and it does
// so from a process that was never told to.
//
// A proxy named for this wallet stops it too, on a weaker argument — not that a proxy could
// carry the hyperdht, but that the updater is traffic nobody asked for, and someone who has
// gone to the trouble of naming a proxy is not looking for a background process opening
...
export function updaterBlocked() {
  ...
  if (bound) return `--dht-interface ${boundVia}`
  return null
}
