...
//
// --interface is the opposite: it is a claim about one host, and Bare's TCP stack has no way
// to bind an outgoing connection to a local address. So it can hold for the hyperdht (a UDP
// socket, which can be bound) and for nothing that goes out over TCP — and a request that
// quietly ignored it would be the leak the flag was reached for. That one fails closed.
//
...
// Fail closed on the TCP side of --interface, because Bare offers no way to say which
// address an outgoing connection leaves from and a request that quietly ignored the flag
// would be exactly the leak the flag was reached for.
//
// This is the second line rather than the first. bin.mjs turns --interface away at the top
// of every command but `give --dht` and `get --dht`, so by the time anything here opens a
// socket the flag has already been ruled out — the fetch wrapper below and
// lib/websocket.mjs call this against the day that stops being true.
//
// lib/lan.mjs opens TCP as well and calls neither, since `--lan` is one of the commands
// bin.mjs has already refused. Its own header says why pinning three of its four sockets
// and not the fourth would be worse than pinning none.
...
// process that fetches over the hyperdht and has no idea any of this was asked for.
//
// --interface stops it because its traffic would go out from an address the user did not
// choose. A proxy named for this wallet stops it too — not because a proxy could carry the
// hyperdht, but because the updater is traffic nobody asked for, and someone who has gone
// to the trouble of naming a proxy is not looking for a background process making
// connections of its own. A proxy merely inherited from the environment says nothing about
// any of that and leaves the updater alone.
export function updaterBlocked() {
  if (explicit) return (https ?? http).source
