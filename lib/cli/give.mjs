...
import { findPeer as findOnLan } from '../lan.mjs'
import { wireIdentity } from './address.mjs'
import { transportFrom, WIRE, DHT, LAN } from './transport.mjs'
...
export async function run({ dir, flags, command }) {
  ...
  // Which of the three ways to reach someone who is not standing here: a radio link to a
  // neighbour, a link over the wi-fi both are on, or a holepunched one to a peer anywhere.
  const transport = transportFrom(flags)
  const over = WIRE[transport]
  ...
  // Same handover over every wire; only finding the receiver differs. The hyperdht also
  // has us present a key to them — a key of this run's own, unless --stable asks for this
  // wallet's address (see address.mjs) — which neither of the others has an equivalent of,
  // so the choice is made only on that branch.
  // Same identity whichever wire carries it: every one of them ends in the same Noise
  // handshake, so the key the receiver sees is ours to choose here rather than the
  // transport's to invent (see lib/cli/address.mjs).
  const keyPair = wireIdentity(wallet, flags)
  const reach =
    transport === DHT
      ? (key, opts) => findPeer(key, { ...opts, keyPair })
      : transport === LAN
        ? (key, opts) => findOnLan(key, { ...opts, keyPair })
        : (key, opts) => findNeighbour(key, { ...opts, keyPair })
