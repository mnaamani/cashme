...
// Bluetooth and the local network have no such choice: their keys are always for one run
// (see lib/ble.mjs and lib/lan.mjs).
import { dhtAddress, ephemeralAddress } from '../dht.mjs'
import { seedFromHex } from '../seed.mjs'
import { note } from '../notes.mjs'
import { DHT, WIRE } from './transport.mjs'
...
// --stable says nothing on the other two wires, where every key is already for one run and
// there is no wallet address to present. Said out loud rather than ignored, so the flag is
// not read as having done something.
export function warnStable(flags, transport) {
  if (!flags.stable || transport === DHT) return
  note(`--stable is redundant without --dht: the ${WIRE[transport]} key is new every run`)
}
