...
export async function run({ dir, flags, command }) {
  ...
  // Parsed before anything is looked up: a mistyped note id is a typo, and it costs
  // nothing to find that out here.
  const noteId = flags.event ? parseNoteId(flags.event) : null

  const target = await findEndpoint(dir, flags)
  const targeted = noteId && (await checkNote(noteId, target))
  ...
  const zapRequest = await buildZapRequest(target, params, { msats, flags, note: targeted })
...
// The note this zap is aimed at, read back from the relays and checked to be the
// recipient's own before an invoice is even asked for.
//
// A zap request is what carries the `e` tag, so a payment that cannot be one has nowhere to
// put a note: paying anyway would send the sats and silently drop the attribution that was
// the reason for naming a note at all. Better to stop while nothing has been spent.
async function checkNote(target, endpoint) {
  if (!endpoint.recipient) {
    throw new Error(
      `there is no nostr key behind "${endpoint.label}", so this payment cannot carry a ` +
        'note tag — pay them without naming a note, or zap their npub instead'
    )
  }

  const pool = new RelayPool([...target.relays, ...endpoint.relays])
  try {
    note(`Reading note ${target.id.slice(0, 12)}…`)
    const event = await readNote(pool, target, endpoint.recipient)
    note(`  ${notePreview(event)}`)
    return event
  } finally {
    pool.destroy()
  }
...
async function buildZapRequest(target, params, { msats, flags, note: targeted = null }) {
...
export async function planZap(wallet, { pubkey, amount, comment = '', event = null, relay = [] }) {
  ...
  const noteId = event ? parseNoteId(event) : null
  const target = await findEndpoint(wallet.dir, { pubkey, relay })
  // Checked before the invoice is asked for, and before any of it is paid: a zap under the
  // wrong note is not something a later run can move.
  const targeted = noteId && (await checkNote(noteId, target))
  const params = await fetchPayParams(target.endpoint)
  ...
  return {
    ...
    // Already checked to be theirs; kept so the caller can show which note it was.
