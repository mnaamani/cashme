...
import { payBolt11 } from './withdraw.mjs'
...
export async function run({ dir, flags, command }) {
  ...
  const target = await findEndpoint(flags)
...
async function findEndpoint(flags) {
  ...
  let hinted = []
  try {
    if (address) console.log(`Resolving ${typed}`)
    const resolved = await resolveRecipient(typed)
    recipient = resolved.pubkey
    hinted = resolved.relays
    if (address) console.log(`  ${typed} is ${recipient}`)
  } catch (err) {
    // Only an address has anywhere left to go: a key that will not parse is a typo.
    if (!address) throw err
    console.error(`  no nostr address there (${err.message})`)
    console.error('  paying it as a plain lightning address — there will be no zap receipt')
  }
  ...
  if (listed) {
    console.log(`  they receive at ${profile.lud16 ?? 'an lnurl in their profile'}`)
    return {
  ...
  if (isLightningAddress(typed)) {
    console.error('  their profile lists no lightning address; paying the address you typed')
...
async function buildZapRequest(target, params, { msats, flags }) {
  ...
  if (!params.allowsNostr) {
    ...
    console.error('The payment would go through as an ordinary lightning payment instead:')
    console.error('nothing would appear on nostr, and nobody would see who paid.')
