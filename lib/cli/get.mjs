...
export async function run({ dir, flags }) {
  ...
  // Either wire, same shape: listen, hand each token to the wallet, stop when the user does.
  ...
  const listen = flags.dht
    ? (opts) =>
        receiveOverDht({ keyPair: dhtIdentity(wallet, flags, { listening: true }), ...opts })
    : receiveTokens
  await listen({
...
// read from forever waiting for a token nobody is typing — hence --bluetooth and --dht,
// either of which says this run listens regardless of what stdin looks like.
//
...
async function tokenFromArgs(flags) {
  ...
  if ((flags.bluetooth || flags.dht) && flags.token !== '-') return null
