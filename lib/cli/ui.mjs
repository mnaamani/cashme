...
// stdout, and the deposit poll writes its dots the same way, where console.error and
// process.stderr do not interleave in order.
//
...
export function showQr(text, { ecc = QR_ECC } = {}) {
  ...
  if (width > columns) {
    console.error(`(QR needs ${width} columns, this terminal has ${columns})`)
    return false
  }
  // Two rows of modules per line of text, so the code stays roughly square.
  const qr = renderUnicodeCompact(text, { border: QR_BORDER, ecc })
...
export function showToken(token, { qr = false } = {}) {
  ...
  console.error('the token is too long to show as a QR here — copy the text above instead')
  return false
}
