...
import { balances, totalBalances, normalizeUnit, knownMints, DEFAULT_UNIT } from '../manager.mjs'
...
// Which mint to act at, which this wallet never picks for you.
//
// Naming a mint is the decision to trust it — to hold a claim against whoever runs it, and
// to let a later send be funded from it (see `cashme mints`). A default made that decision
// silently, on the run where it matters most: the one putting money in. And the default was
// testnut, whose invoices pay themselves and whose ecash is worthless, so the failure was
// quiet in both directions — a real deposit aimed at a toy mint, or a toy balance mistaken
// for a real one.
//
// The mints already trusted are named in the error, since one of them is usually the answer
// and nobody should have to go and look it up.
export async function mintFlag(wallet, value) {
  if (value) return value
  const trusted = (await knownMints(wallet)).filter((mint) => mint.trusted)
  throw new Error(
    trusted.length
      ? `--mint is required — this wallet trusts ${trusted.map((mint) => mint.mintUrl).join(', ')}`
      : '--mint is required — this wallet trusts no mints yet, and ' +
          '`cashme mints --trust <url>` adds one'
  )
}
