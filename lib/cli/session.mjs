...
export async function useWallet(dir) {
  ...
  // openWallet hands back proofs held by sends and payments that never went through, and
  // finishes deposits a run died in the middle of (see sweepPreparedOperations and
  // settleInFlightMints). Both change the balance about to be shown, so say so.
  ...
  for (const deposit of opened.deposited) {
    note(`finished a deposit of ${deposit.amount} ${deposit.unit} that was interrupted`)
  }
  return opened
}
...
//
// Every command that waits does so without a timeout, so Ctrl-C is the normal way out, and
// what it costs depends on where the wait is. `give` waiting for a neighbour has proofs
// reserved and nothing sent, so it hands them back; `give` waiting for a handed-over token
// to be claimed leaves the send pending, because the token is already out there; `get` just
// stops listening. `release()` restores the default, so a second Ctrl-C behaves as usual.
//
