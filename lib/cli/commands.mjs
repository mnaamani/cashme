...
export const root = command(
  ...
  description(
    [
      ...
      '',
      'A proxy carries what a proxy can carry: http, https and the relay websockets. The',
      'hyperdht holepunches over UDP and --lan finds its peer by multicast, so neither goes',
      'through it — `give --dht` behind a proxy swaps at the mint through the proxy and',
      'hands the token over directly, and says so. --interface is the stricter of the two:',
      'Bare cannot bind an outgoing TCP connection to an address, so it holds on the',
      'hyperdht and nowhere else, and a command that would have to ignore it stops instead.',
