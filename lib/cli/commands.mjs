...
export const give = command(
  ...
  flag('--stable|-s', "send under this wallet's own hyperdht address rather than a one-run key"),
...
export const get = command(
  ...
  description(
    [
      ...
      'hyperdht, where the sender can be anywhere. Both keys are new every run, so the',
      "sender needs the current one. --stable listens on this wallet's own hyperdht address",
      'instead, the same every run so a sender can keep it — which also makes it something',
      'anyone holding it can watch for. A token pasted or piped in is claimed on the spot:',
      '',
  ...
  flag('--stable|-s', "listen on this wallet's own hyperdht address rather than a one-run one")
)
...
export const root = command(
  ...
  description(
    [
      ...
      '                                    same thing for every run.',
      '',
      '  --interface en0                   pin outgoing packets to one local address,',
      '                                    named by interface or by the address itself.',
      '',
      'Neither flag half-applies. A proxy cannot carry the hyperdht, since `give --dht`',
      'and `get --dht` holepunch over UDP; --interface can hold there and nowhere else,',
      'since Bare cannot bind an outgoing TCP connection to an address. A command that',
      'would have to go out some other way than the one asked for stops and says so.',
      'Bluetooth and `give --print` touch no network at all.'
    ].join('\n')
  ),
  flag('--version|-v', 'Print the current version'),
  flag('--storage <dir>', 'custom storage directory'),
  flag('--proxy <url>', 'send every http(s) and relay connection through this proxy'),
  flag('--interface <name|ip>', 'bind hyperdht traffic to this interface or local address'),
