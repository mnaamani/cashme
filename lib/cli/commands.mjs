...
export const deposit = command(
  ...
  // without this the paragraph below is printed inside `cashme --help`'s command list.
  ...
  //
  // Which mint to use is deliberately not answered here. Every mint is custodial — the
  // operator holds the bitcoin backing our ecash and can walk off with it — so naming a few
  // reads as an endorsement of them however it is worded, and a list compiled once goes
  // stale in a way nobody running this binary can see. Where to look is the durable half of
  // that advice, so it is the half that is printed.
  description(
    [
      'Every mint is custodial: the operator holds the bitcoin backing your ecash and can',
      'refuse to pay it out, so keep balances small and spread across mints you chose.',
      'Reviews at bitcoinmints.com, proof-of-liabilities audits at audit.8333.space.',
      '',
      '`cashme mints` lists the ones this wallet already trusts.'
    ].join('\n')
  ),
  flag('--amount|-a <amount>', 'how much to deposit, counted in --unit'),
  flag('--mint|-m <url>', 'mint to deposit at (required — this wallet picks no default)'),
...
export const nutzap = command(
  ...
  flag('--event|-e <id>', 'note being zapped: note1…, nevent1…, or 64 hex characters'),
  flag('--yes|-y', 'skip the confirmation prompt')
)
...
export const mints = command(
  'mints',
  summary('List the mints this wallet trusts, and trust or untrust one'),
  description(
    [
      'A mint holds the bitcoin backing our ecash. Trusting one means holding a claim',
      'against whoever runs it, and means a later send may be funded from it — so this is',
      'the list of who this wallet is exposed to, in one place.',
      '',
      'Untrusting does not remove the mint or the ecash at it. Nothing can be spent from',
      'it until it is trusted again, which makes it the way to quarantine a mint without',
      'losing what is there.'
    ].join('\n')
  ),
  flag('--trust <url>', 'trust this mint, adding it if the wallet has never used it'),
  flag(
    '--untrust <url>',
    'stop trusting this mint, keeping its ecash unspendable until it is trusted again'
  ),
  flag('--yes|-y', 'skip the confirmation when untrusting a mint that holds something')
)

export const relays = command(
  'relays',
  summary('List the nostr relays this wallet uses, and add or remove one'),
  description(
    [
      'Only `zap` and `nutzap` touch nostr, and both start by asking relays a question:',
      'where a user receives lightning, how they want to be nutzapped, and — for a nutzap —',
      'where to publish the ecash. This is the list of relays that gets asked.',
      '',
      'Until something is changed here the list is the one built into this binary. The',
      'first change starts from those, so removing one leaves the rest rather than emptying',
      'the list, and --reset goes back to the built-in list whatever it becomes.',
      '',
      'A relay sees which keys this wallet asks about and, for a nutzap, the event carrying',
      'the ecash — see `docs/usage/where-the-traffic-goes.md`. --relay on zap and nutzap is',
      'still there for a relay wanted once rather than kept.'
    ].join('\n')
  ),
  flag('--add <url>', 'use this relay as well, adding it to the list'),
  flag('--remove <url>', 'stop using this relay'),
  flag('--reset', 'go back to the relays built into this binary')
)
...
export const restore = command(
  ...
  flag('--mint|-m <url>', 'mint to restore from (required — this wallet picks no default)')
)
...
export const root = command(
  ...
  flag('--no-updates', 'disable OTA updates for this run (CASHME_NO_UPDATES for every run)'),
