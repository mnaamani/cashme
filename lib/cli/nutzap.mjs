...
//
// Not a NIP-57 zap. A zap is a lightning payment with a nostr receipt, and would be `pay`
// with an LNURL lookup in front of it. A nutzap moves the ecash itself: the proofs are
// locked to the recipient's public key (NUT-11 P2PK) and carried in the tags of a kind
// 9321 event, so there is no invoice, no route, and nothing for us to pay a routing fee on.
...
async function nutzap(wallet, pool, { recipient, amount, flags }) {
  ...
  console.log(`Looking up ${recipient.slice(0, 12)}… on ${pool.relays.length} relays`)
  ...
  // NIP-61 locks to the pubkey in the profile, which is a 33-byte cashu key, not the
  // x-only nostr one. Without a profile the convention is the nostr key with an even-y
  // prefix, which is what the recipient's wallet will derive too.
  const lockKey = info?.pubkey ?? `02${recipient}`
  const relays = info?.relays?.length ? info.relays : pool.relays.map((relay) => relay.url)
  ...
  const prepared = await prepareP2pkSend(wallet, mintUrl, amount, lockKey)
  ...
  try {
    ...
    if (!results.some((result) => result.ok)) {
      ...
      console.error('cannot be reclaimed — publish this event yourself to deliver it:\n')
...
async function readNutzapInfo(pool, recipient) {
  ...
  for (const mint of tagValues(event, 'mint')) {
    try {
      ...
      console.error(`ignoring an unusable mint in their profile: ${mint}`)
    }
  }
...
async function chooseMint(wallet, amount, info, flags) {
  ...
  if (flags.mint) {
    ...
    if (trusted.length && !trusted.includes(mintUrl)) {
      console.error(`Warning: they did not list ${mintUrl} as a mint they trust.`)
      console.error(`They listed: ${trusted.join(', ')}`)
