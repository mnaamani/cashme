...
export async function planNutzap(wallet, { pubkey, amount, mint = null, relay = [] }) {
  const warnings = []
  const { pubkey: recipient, relays: hinted } = await resolveRecipient(pubkey)

  const pool = new RelayPool(relaysFor(wallet.dir, { hinted, extra: relay }))
...
export async function settleNutzap(wallet, plan, { comment = '', event = null } = {}) {
