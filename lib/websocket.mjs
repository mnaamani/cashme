...
import { requestOptions, assertUnbound } from './net.mjs'
...
export class BareWebSocket {
  ...
  constructor(url) {
    ...
    //
    // The --interface check is belt and braces. bin.mjs refuses every command but
    // `give --dht` and `get --dht` while the flag is set, and neither of those reaches a
    // relay — only `zap` and `nutzap` do — so this cannot fire as things stand. It stays
    // because what makes it necessary belongs to the socket rather than to the command:
    // Bare cannot bind an outgoing TCP connection to an address, so a relay opened from
    // somewhere new has to fail closed rather than inherit an exemption nobody rechecked.
    assertUnbound('nostr relays')
