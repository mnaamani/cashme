...
export class BareWebSocket {
  ...
  constructor(url) {
    this.url = url
    // A relay is reached over the same TCP as a mint, so it goes through --proxy the same
    // way and is refused by --interface the same way. bare-ws hands its options straight to
    // bare-http1, which is where the agent belongs.
