...
//
// Every socket here binds 0.0.0.0, and --interface is refused for `--lan` in bin.mjs rather
// than honoured. Not an oversight: three of the four sockets could be pinned — the discovery
// socket, its group membership and the receiver's listening socket all take a local address
// — but the handover itself is `tcp.connect` below, and Bare's TCP stack has no way to say
// which address an outgoing connection leaves from. Pinning the beacon while the ecash goes
// out from wherever the routing table decides is the half-applied flag lib/net.mjs exists to
// refuse, so the whole transport is refused instead.
//
