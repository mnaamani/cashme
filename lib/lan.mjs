...
//
// Every socket here that binds at all binds 0.0.0.0, and --interface is refused for `--lan`
// in bin.mjs rather than honoured. Not an oversight: three of the four could be pinned — the
// two discovery sockets take a local address, the receiver's group membership names an
// interface, and its listening socket takes an address too — but the fourth is the
// `tcp.connect` below that carries the token, and Bare's TCP stack has no way to say which
// address an outgoing connection leaves from, so that one binds to nothing. Pinning the
// beacon while the ecash goes out from wherever the routing table decides is the
// half-applied flag lib/net.mjs exists to refuse, so the whole transport is refused.
//
// It is nearly true that the connect follows the beacon: the peer is on-link, so the
// connected route wins and the source is the address we bound. Nearly, because two
// interfaces on one subnet (a docked laptop on ethernet and wi-fi at once) give the kernel
// two routes for that prefix and it picks by metric, not by which one heard the beacon. That
// gap can be closed — check the subnet is unambiguous before dialling, check
// `socket.localAddress` after — but the SYN of a failed check has already left, and udx
// exposes no IP_MULTICAST_IF to pin the query's egress either. A guarantee with footnotes is
// not what --interface promises, so it stays a hyperdht flag, where the whole conversation
// is UDP and every socket in it can be bound.
...
export function findPeer(pubKey, { cancelled } = {}) {
  ...
  const finding = new Promise((resolve, reject) => {
    ...
    socket.on('message', (message, rinfo) => {
      ...
      // Our key is this run's alone. Nothing on this transport keeps one — there is no
      // address here to be recognised by later, the way `--dht --stable` deliberately is.
      const conn = new NoiseSecretStream(true, link, { keyPair: crypto.keyPair() })
...
//
// Same shape as the other two listeners: links are handled concurrently — each sender gets
...
export async function receiveTokens({ ontoken, cancelled } = {}) {
  // One key per run, like bluetooth: it is how the sender picks us out of the room for the
  // next few seconds and is worth nothing afterwards.
  ...
  const server = tcp.createServer((socket) => {
    ...
    const conn = new NoiseSecretStream(false, socket, { keyPair })
  ...
  const reply = beacon(keyPair.publicKey, port)
  socket.on('message', (message, rinfo) => {
    const packet = parse(message)
