...
export class FileRepositories extends MemoryRepositories {
  ...
  init() {
    try {
      // 0700 to match the wallet file: the seed inside it is the wallet, and a directory
      // listing should not be the thing that leaks its name and size to other users.
  ...
  close() {
    // A failed final write must still let another process open the wallet. `flush()` keeps
    // the repository dirty so a live caller can report or retry the failure, but holding the
    // advisory lock after throwing would turn that recoverable disk problem into a stranded
    // wallet until this process is killed.
    try {
      this.flush()
    } finally {
      this.lock.release()
  ...
  async withTransaction(fn) {
    ...
    try {
      ...
      note('[wallet] could not write the rolled back state:', err.message)
    }
  ...
  // Write beside the wallet, then rename over it: a reader never sees half a file, and a
  // crash leaves either the old wallet or the new one.
  //
  // Both halves are fsynced, which a plain write is not. Without it the rename can reach
  // the disk before the bytes it points at, and a machine that loses power mid-command
  // comes back to a wallet file that is empty or stale — proofs the mint has already
  // issued against, missing. This costs a few milliseconds per write; the proofs are
  // worth more.
  save() {
    if (!existsSync(this.dir)) mkdirSync(this.dir, { recursive: true, mode: 0o700 })
    ...
    try {
      ...
      // writeSync is allowed to write less than it was given, however unlikely that is
      // for a local file — the remainder would be a wallet truncated mid-json.
      for (let written = 0; written < bytes.byteLength; ) {
        written += writeSync(file, bytes.subarray(written))
      }
      fsyncSync(file)
    } finally {
      closeSync(file)
...
// A rename is a directory change, and durable only once the directory itself is synced.
// Not every platform lets a directory be opened — Windows does not — so this is the one
// part of the write that is allowed to fail quietly: the file's own contents are already
// safe, which is the half that matters.
