...
// The runtime being expanded right now, and where in the tree. Module state rather than an
// argument, because hooks are called by the component, which is given only its props.
let current = null
...
class Runtime {
  ...
  paint(tree) {
    ...
    // One line short of the terminal's height: writing the last cell of the last row makes
    // some terminals scroll, which on the alternate screen shifts the whole frame up.
    const lines = fit(layout(tree, columns), Math.max(1, rows - 1), columns)
...
export function useSize() {
  ...
  const [size, setSize] = useState(() => ({
    columns: runtime.stdout.columns || 80,
    rows: runtime.stdout.rows || 24
  }))
  useEffect(() => {
    const onresize = () =>
      setSize({ columns: runtime.stdout.columns || 80, rows: runtime.stdout.rows || 24 })
