...
function renderRow(node, columns) {
  ...
  if (slack > 0 && totalGrow > 0) {
    // Proportional shares, floored — and the rounding those floors leave over goes to the
    // last growing child, so the row comes to exactly `columns` rather than a column short.
    const growing = grow.map((factor, index) => (factor ? index : -1)).filter((i) => i >= 0)
    let handed = 0
    growing.forEach((index, nth) => {
      const share =
        nth === growing.length - 1 ? slack - handed : Math.floor((slack * grow[index]) / totalGrow)
      widths[index] += share
      handed += share
    })
  } else if (slack < 0) {
