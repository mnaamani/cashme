...
// `splash` is a prop rather than a constant so a test can mount straight into the wallet:
// the animation is a second of the screen deliberately showing something other than what
// the test came to look at, and skipping it with a keystroke would spend a keystroke.
export function App({ api, version = '', ephemeral = false, splash = true }) {
  const [route, setRoute] = useState('menu')
  const [booting, setBooting] = useState(splash)
  ...
  // The wordmark takes the whole frame rather than sitting above a half-built dashboard:
  // it is the one moment this UI is allowed to be only a picture, and a header and an empty
  // log pane around it would make it a loading screen instead. Every hook above has already
  // run, so this return costs the tree nothing on the frame the animation ends.
  if (booting) {
    return column(
      { height: rows },
      h(Splash, { columns, height: rows, version, onDone: () => setBooting(false) })
    )
  }
...
//
// The proxy is up here for the same reason: it is true of every screen, and which wire the
// mint and relay traffic is leaving by is the sort of thing that should not have to be
// looked up in settings to be believed. Dim, because unlike the address it is decided on
// the command line and cannot be changed from in here.
//
// The dev badge is there for the same reason, one step earlier: a dev build with no
// --storage keeps its wallet in a temp directory, so a balance this wallet shows is a
// balance that goes away with the machine's next clear-out. Nothing else on screen distinguishes
// that from real money, so it is said in the one place that is true of every screen.
//
// What is deliberately not up here is the balance: the menu holds the total and the mints
// screen breaks it down, and a number repeated in two places is a number that can disagree
// with itself.
function TopBar({ route, version, error, address, ephemeral, proxy }) {
  const stable = address === 'stable'
  return row(
    { gap: 2 },
    h(Mark, { label: `cashme${version ? ` ${version}` : ''}` }),
    ephemeral ? text(' dev · temp storage ', { bold: true, inverse: true, yellow: true }) : null,
    text(TITLES[route] ?? route, { dim: true, grow: 1 }),
    proxy ? text(`via ${proxy.name}`, { dim: true }) : null,
    text(address, { yellow: stable, dim: !stable }),
    error ? text(`✗ ${error.message}`, { red: true }) : null
  )
}
