...
export function App({ api, version = '' }) {
  ...
  // Only the two things that are not actions. Everything the wallet can do is chosen from
  // the menu's list, so it handles its own arrows and return; these are what is left over,
  // and they are live only while the menu is what is on screen.
  useInput(
    (key) => {
      if (route !== 'menu') return
      const pressed = key.input?.toLowerCase()
      if (pressed === 'q' || key.name === 'ctrl-c') return exit()
...
function TopBar({ route, totals, version, error, address }) {
