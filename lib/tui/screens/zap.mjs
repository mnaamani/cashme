...
const FIELDS = ['pubkey', 'amount', 'comment']
const SLOTS = FIELDS.length + 1
const BUTTON = FIELDS.length

export function Zap({ api, onBack, onChanged, columns = 80 }) {
  const [form, setForm] = useState({ pubkey: '', amount: '', comment: '' })
  const [at, setAt] = useState(0)
  const [plan, setPlan] = useState(null)

  const quote = useTask(async ({ pubkey, amount, comment }, { say }) => {
    say(`looking up ${pubkey}`)
    const planned = await api.planZap({ pubkey, amount, comment })
    setPlan(planned)
  ...
  const asking = Boolean(plan?.melt?.payable) && !pay.busy && pay.status === 'idle'
  ...
  useInput(
    (key) => {
      ...
      if (key.name === 'return' && at === BUTTON) {
        if (ready) {
          quote.run({ pubkey: form.pubkey.trim(), amount, comment: form.comment.trim() })
        }
        return
      }
