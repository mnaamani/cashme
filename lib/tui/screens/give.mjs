...
export function Give({ api, columns, height = 24, onBack, onChanged }) {
  const [form, setForm] = useState({ amount: '', key: '', mint: '' })
  ...
  useInput((key) => {
    // Before the busy check, not after it. On the `hand over` path the token is on screen
    // for the whole of the wait for it to be claimed — the task is busy that entire time —
    // and that wait is exactly when somebody wants the token out of here. Behind the check
    // this key did nothing in the one window it was for.
    if (key.input?.toLowerCase() === 'c' && token) {
    ...
    if (key.name === 'return' && at === button) {
      ...
      setToken(null)
      setCopied(null)
      send.run({ amount, key: form.key.trim(), mint: form.mint.trim() || null })
      return
    }
...
function TokenPane({ token, code, columns, height, copied, task }) {
  ...
  return column(
    ...
    h(Qr, {
      ...
      columns,
      // The form above takes four methods, up to three fields, a button, a status line and
