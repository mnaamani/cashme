...
export function Settings({ api, onBack, columns = 80, focus = true }) {
  ...
  const rows = [
    ...
    { label: 'binary', value: settings.binary },
    {
      ...
      note: 'everything this wallet keeps on disk is under here'
    },
    {
      label: 'wallet file',
      value: settings.wallet,
      // Asked for as “the wallet and the seed file” and answered as one line, because
      // there is only one file: the NUT-13 seed is a field inside wallet.json rather than
      // a file beside it. Worth saying on the screen that shows the path — someone backing
      // up “the seed file” would otherwise go looking for one that does not exist.
      note: 'proofs, history and the NUT-13 seed — the seed is inside this file, not beside it'
    },
    {
      label: 'proxy',
