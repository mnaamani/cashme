...
import { Crest, glow } from '../brand.mjs'
import { Total } from './mints.mjs'
...
//
// Three groups, because they are three kinds of thing and a flat list of ten makes the
...
const GROUPS = [
  [
    { route: 'settings', label: 'settings', hint: 'current session configuration' },
    { route: 'mints', label: 'mints', hint: 'trusted mints, and proofs for each' },
    { route: 'relays', label: 'relays', hint: 'the nostr relays this wallet queries' },
    { route: 'inflight', label: 'in flight', hint: 'sends not yet known to be claimed' }
  ],
  [
    { route: 'deposit', label: 'deposit', hint: 'mint ecash with a lightning payment' },
    { route: 'give', label: 'give', hint: 'send ecash to someone' },
    { route: 'get', label: 'get', hint: 'get ecash from someone' },
    { route: 'withdraw', label: 'withdraw', hint: 'spend ecash, pay a lightning invoice' }
  ],
  [
...
// What this screen costs before anything decorative is put on it: the balance pane, and
// the actions pane with a row per action and a blank between the groups. Whatever the
// screen is given beyond this is room the crest is allowed to ask for — and it is worked
// out from the list rather than written down, so adding an action cannot silently push the
// last one off the bottom of a short terminal.
const BASE_ROWS = 3 + 2 + GROUPS.flat().length + (GROUPS.length - 1)

export function Menu({ snapshot, columns = 80, height = 0, onOpen, from = null, focus = true }) {
  ...
  return column(
    { gap: 0, grow: 1 },
    h(Crest, { columns, room: height - BASE_ROWS }),
    h(Total, { snapshot }),
    h(
      Panel,
      { title: 'actions', focus, grow: 1 },
      h(Select, { items: shown, index, focus, labelWidth: LABEL_WIDTH, paint: glow })
    )
  )
}
