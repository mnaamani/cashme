// Get: take a token that was pasted in, or wait on a wire for one. Named for the command
// it is the other face of — `cashme get` does exactly this one token at a time.
//
...
import { useState, useInput, useRef } from '../runtime.mjs'
import {
  Panel,
  Field,
  Button,
...
export function Get({ api, onBack, onChanged, columns = 80 }) {
  const [stage, setStage] = useState(SOURCE)
  const [at, setAt] = useState(0)
  const [source, setSource] = useState(0)
  const [token, setToken] = useState('')
  const [taken, setTaken] = useState([])
  // The same count the pane shows, kept where the listening task can read it. `taken` is
  // the array from the render that started the wire, so it is still empty in there however
  // many tokens went through — a session that took three would have reported none.
      ...
      await api.listen({
        ...
        ontoken: async (received) => {
          try {
            ...
            // Said here, because the pane is where somebody is looking. Then rethrown,
            // which is the part that matters: the sender was acknowledged the moment the
            // token parsed — before this decision was even put — so it has let go of those
            // proofs and this string is the only copy of them left. The wire's queue turns
            // a throw into the token being printed rather than dropped (see
            // lib/token-wire.mjs), and it lands in the log, which is the one pane here that
            // can be copied out of. Swallowing it would make refusing a mint mean losing
            // the money rather than declining it.
            say(`refused: ${err.message}`)
            throw err
          }
        }
      })
      const took = count.current - before
      return took ? `took ${took}` : 'stopped listening'
    },
    {
      onDone: () => {
  ...
  useInput(
    ...
    // Not registered at all while a question stands: the modal has the keyboard.
    { active: !question }
  )

  // A question owns the screen, as it does on the withdraw, zap and nutzap screens — and
  // with more reason than any of them. This is the one that decides whether a stranger's
  // mint goes into this wallet for good, and it used to be drawn last, under a source list,
  // a status line and the address pane: on anything but a tall terminal it was cut to a
  // border with nothing inside it, leaving `Y to trust` on the hint line as the only trace
  // of what was being agreed to. A confirmation whose question is not on screen is not a
  // confirmation.
  if (question) {
    return column(
      { gap: 0, grow: 1 },
  ...
  return column(
    ...
    h(
      Panel,
      { title: 'get', focus: !take.busy },
      h(Select, { items: SOURCES, index: source, focus: stage === SOURCE && !take.busy }),
      text(''),
      wire === 'paste'
        ? h(Field, {
            label: 'token',
            value: token,
            focus: stage === ENTRY && at === 0 && !take.busy,
            placeholder: 'cashuB…'
          })
