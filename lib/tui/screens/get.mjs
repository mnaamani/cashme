      ...
      await api.listen({
        ...
        ontoken: async (received) => {
          try {
            ...
            // Said here, because the pane is where somebody is looking. Rethrowing withholds
            // the wire ACK: the sender cannot finalize until this wallet has stored the
            // token, so declining a mint leaves its proofs with the sender to reclaim.
