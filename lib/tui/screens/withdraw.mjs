...
export function Withdraw({ api, snapshot, onBack, onChanged, columns = 80 }) {
  ...
  const completion = at === 1 ? completes(mint, snapshot?.mints ?? []) : ''
  ...
  useInput(
    (key) => {
      ...
      if (accepts(key, completion)) return setMint(completion)
