...
import { WALLET_FILE } from '../constants.mjs'
...
export function createApi(wallet, { dir } = {}) {
  ...
  return {
    ...
    // The nostr relays `zap` and `nutzap` look on, and the three ways to change the list.
    // Not serialized and not on the wallet: the list lives in its own file beside the
    // wallet, so none of this touches coco or waits behind a payment (see lib/relays.mjs).
    relays: () => readRelays(dir),
    addRelay: (url) => addRelay(dir, url),
    removeRelay: (url) => removeRelay(dir, url),
    resetRelays: () => resetRelays(dir),
    ...
    planZap: ({ pubkey, amount, comment, event }) =>
      serial(() => planZap(wallet, { pubkey, amount, comment, event })),
    settleZap: (plan) => serial(() => settleZap(wallet, plan)),

    planNutzap: ({ pubkey, amount, mint, event }) =>
      serial(() => planNutzap(wallet, { pubkey, amount, mint, event })),
    ...
    settings: () => {
      ...
      return {
        ...
        storage: dir,
        // The one file in that directory that is the money. Named in full rather than left
        // to be assembled from the directory above it: it is the file to copy, and the file
        // whose loss is the loss of the wallet.
        wallet: path.join(dir, WALLET_FILE),
    ...
    // The proxy this run is going out through, or null. Fixed for the run, but read
    // through here rather than imported by the screens so the UI has one api to fake.
    proxy: proxyInForce,
