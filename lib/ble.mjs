...
// how long to let the radio settle after a flush before tearing down
const SEND_GRACE = 2000
...
async function teardown(swarm) {
  ...
  await swarm.close().catch((err) => console.log(err))
      ...
      const handle = openChannel(conn, {
        onopen() {
          ...
          resolve(async (payload) => {
            ...
            conn.end()
            // flush() resolves once every chunk has gone through the BLE pipe's
            // _write. 'finish' is too early (it only ends the raw stream) and
            // 'close' is too late (it needs the remote to confirm, which BLE
            // doesn't reliably deliver — that stalls for the 15s idle timeout).
            await conn.flush().catch((err) => console.log(err))
            // flush confirms the bytes left our side, not that they arrived;
            // there is no ACK in this protocol, so give the radio a moment
            // before killing the managers out from under the transfer.
            await new Promise((r) => setTimeout(r, SEND_GRACE))
            shutdown()
          })
        }
      })
    ...
    function onconnection(conn) {
      ...
      const handle = openChannel(conn, {
        ontoken(token) {
          ...
          // let our own flush (channel-close frame + end) go out before
          // tearing down the swarm, rather than destroying mid-flush
          conn
            .flush()
            .catch(() => {})
