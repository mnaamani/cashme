...
export async function findNeighbour(pubKey) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function shutdown() {
      ...
      // close (not stop): stop() only suspends the transport, keeping the
      // native BLE managers alive for a future resume. This is a one-shot
      // CLI run, so we need the full teardown or the process never exits.
      swarm.close().catch((err) => console.log(err))
    }
      ...
      const handle = openChannel(conn, {
        onopen() {
          ...
          resolve((payload) => {
            ...
            conn.end() // flush
            // don't wait for 'close' (needs the remote to confirm too) — BLE
            // liveness is timeout-based and disconnects aren't reliably
            // delivered, so that can stall for the transport's full idle
            // timeout. 'finish' fires once our own write side has flushed.
            conn.once('finish', () => {
              shutdown()
            })
          })
    ...
    function onconnection(conn) {
      ...
      const handle = openChannel(conn, {
        ontoken(token) {
          ...
          // wait for our own flush (channel-close frame + end) to go out
          // before tearing down the swarm, rather than destroying mid-flush
          conn.once('finish', () => done(null, token))
        }
      })
    }
