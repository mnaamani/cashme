...
export async function findNeighbour(pubKey, { cancelled } = {}) {
  ...
  const finding = new Promise((resolve, _reject) => {
    ...
    // A peer is announced once per link, but a link that drops and comes back announces it
    // again — report each key once so a flapping neighbour does not fill the screen.
    ...
    onconnection = function onconnection(conn) {
      ...
      if (!neighbour.startsWith(pubKey) || found) {
        // Reaching a peer we cannot use looks exactly like reaching nobody at all, unless we
        // say so. The receiver's key is fresh on every `cashme get`, so a `--public-key`
        // copied from an earlier run is the usual reason for landing here.
        if (!found && !reported.has(neighbour)) {
          reported.add(neighbour)
          debug(`saw ${neighbour} — not the neighbour we are looking for`)
        }
        conn.destroy()
        return
      }
      ...
      const handle = openChannel(conn, {
        ...
        onopen() {
          ...
          resolve(async (payload) => {
            // Every step of the handoff is timed, because each one can only end in a
            // handshake with the peer or a timeout, and a `give` that will not exit is
            // read by finding the last step that finished.
...
// Listen for tokens until the caller gives up, calling `ontoken` once for each valid one.
//
// A receiver is not a one-shot: several neighbours can be paying us at once, and the run
// that took the first token used to tear the radio down on the rest. Links are therefore
// handled concurrently — every sender gets its ACK as soon as its token parses, without
// waiting on anybody else's — while `ontoken` calls are queued one behind the other, since
// what they do next is swap against a mint through a single wallet.
//
// `cancelled` is a promise that settles when the caller wants to stop listening; the radio
// comes down before this returns.
export async function receiveTokens({ ontoken, cancelled } = {}) {
  ...
  console.error('waiting for tokens — Ctrl-C to stop')

  // Tokens arrive per link and are handled one at a time: coco serialises its own work,
  // but a queue keeps the order the tokens arrived in and keeps one slow mint from
  // interleaving two receives. A handler that throws must not take the listener down with
  // it — the next neighbour is still coming.
  let queue = Promise.resolve()
  function enqueue(token) {
    queue = queue
      .then(() => ontoken(token))
      .catch((err) => {
        console.error('[app:error] could not receive that token:', err.message)
      })
    return queue
  }
  ...
  function onconnection(conn) {
    ...
    const handle = openChannel(conn, {
      ...
      async ontoken(token) {
        ...
        // Hand it on before the goodbye rather than after: the wallet work and this link's
        // hangup have nothing to say to each other, and the sender is waiting on the
        // hangup.
        enqueue(token)
        ...
        // we hang up first; the sender is waiting on the link dropping. Only this link
        // goes: the swarm stays up for whoever comes next.
  ...
  debug('shutting down ble-swarm')
  swarm.off('connection', onconnection)
  // Whatever was still being received keeps the wallet busy after the radio is down; let
  // it finish, or a Ctrl-C mid-swap would close the wallet underneath it.
  await queue
  await teardown(swarm)
}
